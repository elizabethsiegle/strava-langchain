...
# pip install chromadb, tabulate google-search-results reportlab openai
    ...
    if submitted:
        ...
        if VALIDANSWERS:
            ...
            my_dataset = requests.get(
                ACTIVITIES_URL, headers=header, params=params).json()  # activities 1st page
            PAGE = 0
            for x in range(1, 5):  # loop through 4 pages of strava activities
                PAGE += 1
            params = {'per_page': 200, 'page': PAGE}
            # add to dataset, need strava token in .env to be updated else get dict error
            my_dataset += requests.get(ACTIVITIES_URL,
            ...
            # remove items from 2021, only include workouts from 2022 and 2023
            ...
            data_run_df['distance'] = data_run_df['distance'].map(
                lambda x: convert_to_miles(x))
            data_activity_df['distance'] = data_activity_df['distance'].map(
                lambda x: convert_to_miles(x))
            # convert moving time secs to mins, hours
            data_run_df['moving_time'] = data_run_df['moving_time'].astype(
                str).map(lambda x: x[7:])  # slices off 0 days from moving_time
            ...
            tools = [
                ...
                Tool(
                    name="days_between",
                    func=lambda day1, day2=marathon_date: calc_days_btwn(
                        day1, day2),
            ...
            pd_agent_run = create_pandas_dataframe_agent(
                OpenAI(temperature=0), data_run_df, verbose=True)  # csv agent?
            pd_agent_activity = create_pandas_dataframe_agent(
                OpenAI(temperature=0), data_activity_df, verbose=True)
            ...
            num_workouts = agent.run(
                f'Calculate the number of days between these two dates: {marathon_date} and {training_start_date}. You have access to tools')
            pd_run_output = pd_agent_run.run(
                "Calculate average distance and maximum distance in miles and average moving time and maximum moving time in minutes. Additionally, calculate any other running statistics from the data you think would be helpful to consider in marathon training.")
            ...
            example_prompt = PromptTemplate(input_variables=[
                                            "training_start_date", "marathon_date", "pd_run_output", "num_workouts", "plan"], template=coach_template)
            ...
            few_shot_prompt_template = FewShotPromptTemplate(
                ...
                suffix=suffix,
                input_variables=[
                    "marathon_date", "training_start_date", "pd_run_output", "num_workouts"]
            )
            ...
            plan = agent_executor({"marathon_date": marathon_date, "training_start_date": training_start_date,
                                  "pd_run_output": pd_run_output, "num_workouts": num_workouts})
            print('plan being print ', plan)
            plan = plan['output']
            # each workout on new line
            ...
            if response.status_code == 202:
                st.success(
                    "Email sent! Check your email for your personal training plan")
                print(
